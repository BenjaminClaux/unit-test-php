...
class Room
{
    ...
    public function setLevel(string $value)
    {
        ...
        if($value == self::DIFFICULT_LEVEL || $value == self::NORMAL_LEVEL || $value == self::EASY_LEVEL) 
        {
    ...
    public function getNbEnigma(): int
    {
        if ($this->niveau == Room::EASY_LEVEL) {
            return 5;
        } else if ($this->niveau == Room::NORMAL_LEVEL) {
            return 10;
        } else if ($this->niveau == Room::DIFFICULT_LEVEL) {
            return 20;
        }
    }
